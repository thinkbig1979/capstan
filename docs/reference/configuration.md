...
| `RATE_LIMIT_API_PER_MIN` | `300` | No | Per-caller budget for the authenticated API, in requests per rolling minute. Each caller keys on their user ID when authenticated and on the client IP otherwise, so with `AUTH_DISABLED=true` every request shares one bucket. Raise it only where that aggregation is the point, such as an end-to-end test run driving a whole suite through one address; the CI workflow sets `2000`. A non-numeric or non-positive value is a hard startup failure rather than a silent fallback. Login endpoints have their own fixed budgets and are not affected. |
